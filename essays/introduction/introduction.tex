...
Sensitivity, the phenomenon that a small error in the initial state produces an enormous error in later states, occupies a central position in the exposition.
...
Sensitivity has an interesting interplay with noises, which are particularly important in experimental settings.
 inevitable 
Because of this, time series obtained from non-linear systems cannot be analyzed in the same way as those from linear systems are analyzed: 
\begin{quotation}
...
\citet[p.31]{abarbanel,ott1994} state that the two key aspects of chaos are ``the stretching of infinitesimal displacement'' (equivalent to sensitivity) and ``complex orbit structure.'' %abarbanel: p379, ott: p31.
...
\end{quotation}
Listening to the experts' opinions, we have learned two things: that chaos probably consists of complex orbit and sensitivity, and that there is no universally accepted definition of chaos.
