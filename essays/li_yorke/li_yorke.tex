...
His definition requires the mapping to be differentiable.
...
\citet{marotto1} has a definition of chaos, which implies chaos in Li-Yorke's sense, using the notion of \textit{snap-back repeller}.
...
\begin{proposition}
  ...
  Then, there exists a norm defined on $\R^n$ and aneighborhood $N$ of $z$ such that $z$ is an expanding fixed point in $N$ with respect to this norm.
  \begin{proof}
...
Not only the Henon map \citep{marotto1} and some type of wave equation have been shown to be chaotic in Marotto's sense, (Chaotic vibration of the one-dimensional linear wave equation with a van der Pol nonlinear boundary condition),
the definition has been successfully applied to models in ecology and economics.
...
A longer list of applications is in \citet{marotto2}.
