...
\begin{theorem}
  ...
  \begin{proof}
    ...
    and
    \begin{equation*}
      \liminf\limits_{n \to \infty}\; \metric{\itr{F}{n}(p) , \itr{F}{n}(q)} = b.
    \end{equation*}
  ...
  Therefore, for each $\delta$, $G^n(\phi(q)) \in \oball{\delta}{G^n(\phi(p))}$ holds for infinitely many $n$.
  We have $\metric{G^n(\phi(p)), G^n(\phi(q))} \geq 0$, so we conclude that
  \begin{equation*}
    \liminf\limits_{n \to \infty}\; \metric{\itr{G}{n}(\phi(p)) , \itr{G}{n}(\phi(q))} = 0.
  \end{equation*}
  Next, we prove
    \begin{equation*}
      \limsup\limits_{n \to \infty}\; \metric{\itr{G}{n}(\phi(p)) , \itr{G}{n}(\phi(q))} > 0.
    \end{equation*}
    Notice that 
    \begin{equation*}
      G^n(\phi(q)) \not\in \oball{\epsilon}{G^n(\phi(p))}.
    \end{equation*}
...
Although the definition by Li and Yorke captures our intuitive notion of chaos, it is by no means flawless.
