...
\end{equation*}
  ...
  If $0 \in R_\alpha((x, x+\delta])$ or $1 - \alpha \in R_\alpha((x, x+\delta])$, then we can redefine $\delta$ to be a smaller value so that $0, 1-\alpha \not\in R_\alpha((x, x+\delta])$. %)))
  ...
  Clearly, for each $x,y \in \Sigma$, there are infinitely many $n$ such that $\metric{\itr{\sigma}{n}(x),\itr{\sigma}{n}(y)} > 0$.
...
\begin{proposition}
  ...
  If $F$ is topologically transitive, then $F: \bar{X} \to \bar{X}$ is also transitive.
