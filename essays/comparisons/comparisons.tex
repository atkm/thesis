...
This result may turn out to be instructive in evaluating whether \dpp in Devaney's definition is appropriate.
Also, we can make an interesting observation from the fact that \wig and \liy do not imply each other. 
Recall that, though it is not explicitly called sensitivity, Li-Yorke's definition uses its own version of sensitivity, $\limsup\limits_{n\to \infty} \metric{\itr{F}{n}(x), \itr{F}{n}(y)} > 0$, called the \textit{non-asymptotic relation}, which is different from \sdic used in Devaney's and Wiggins's definitions.
...
However, the three definition that are equivalent for a compact inteval are not equivalent in a general compact metric space.
...
If the answer to this question is positive, then we obtain \pte $\Rar$ \wig.
...
There are a lot to ask about Devaney's definition.
Is requiring dense periodic orbits too restrictive? Does periodicity have anything to do with chaos?
We saw that a single periodic point + Wiggins is a sufficient condition for Li-Yorke. 
