...
\begin{figure}[ht]
 ...
 %nodes
    \node[] (dummy) {};
    \node[box, inner sep=5pt,below=1.0cm of dummy] (ly) {Li-Yorke};
    \node[box, inner sep=5pt,right=0.5cm of dummy] (bc) {Block-Coppel};
    \node[box, inner sep=5pt,left=0.5cm of dummy] (dev) {Devaney};
    \node[box, inner sep=5pt,right=1.0cm of bc] (ent) {Entropy};
 ...
 %edges
    ...
    \draw[imply] (wig) to[bend right=20] node {} (ly); 
...
\begin{figure}[ht]
 ...
 %nodes
    \node[] (dummy1) {};
    \node[below=3.0cm of dummy1] (dummy2) {};
    \node[box, inner sep=5pt,below=2.0cm of dummy2] (ly) {Li-Yorke};
    \node[box, inner sep=5pt,left=2.0cm of dummy2] (dev) {Devaney};
    \node[box, inner sep=5pt,right=2.0cm of dummy2] (entropy) {Entropy};
    \node[box, inner sep=5pt,right=1.0cm of dummy1] (bc) {Block-Coppel};
    \node[box, inner sep=5pt,left=1.0cm of dummy1] (wig) {Wiggins};
    \node[below=1.0cm of ly] (dummy3) {};
 %edges
    \draw[imply] (entropy)[bend left=20] to node {} (ly.east); 
    \draw[imply] (dev)[bend right=20] to node {} (ly.west); 
    \draw[imply] (dev)[bend left=15] to node {} (wig); 
    \draw[imply] (bc) to [bend left=15] node {} (entropy); 
    \draw[imply] (bc)[bend right=10] to node {} (ly); 
    \draw[imply] (bc)[bend right=35] to node {} (wig); 
    \draw[maybe] (entropy.east)[swap] to node {?} (bc.east); 
    \draw[maybe] (entropy)[swap,bend right=5] to node {?} (wig); 
...
\citet{glasner} provide an example of a symbolic system that is \dev but has zero entropy.
...
We do not know whether \blcp implies \wig, although we have some related results.
