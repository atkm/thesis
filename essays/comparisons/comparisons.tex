...
\begin{example}
  ...
  \begin{proposition}
    ...
    \begin{proof}
      From the proof that the system is sensitive, we know that there exists $n$ such that $(\itr{R_\alpha}{n}(x), \itr{R_\alpha}{n}(y)]$ contains $0$. %)
      ...
      In particular, we prove that it cannot be the case that the number of time steps between the two such occurences is arbitrarily long.

      First, choose any $x,y \in S^1$.
      Let $b$ be the length of the interval $I \equiv (x,y]$ %)
      First, consider the case $\alpha \leq b$.
      In this case, $I \cap R_\alpha(I)$, so as $I$ rotates around $S^1$, it covers all of $S^1$.
      Let $n$ be the least positive integer such that $n\alpha > 1$.
      Then, it takes no longer than $n$ iterations until $\itr{R_\alpha}{n}(I)$ contains $0$ again.

      Next, consider the other case $\alpha > b$.
      There exists a positive integer $m$ such that $m\alpha \leq b \;(\mod{1})$.
      Then, as in the first case, if we let $n$ be the least positive integer such that $nm\alpha > 1$, then $mn$ is the upper bound for the number of iterations until $\itr{(R_\alpha^{\;m})}{n}(I)$ contains $0$ again.

      Now, consider $s \ceq \phi(x)$ and $t \ceq \phi(y)$.
      By the discussion so far, each block of length $mn$ contains a position $i$ such that $\phi(x)_{i} \neq \phi(y)_{i}$.
