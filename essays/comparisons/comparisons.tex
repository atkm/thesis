...
\subsection*{Devaney/Wiggins and Block-Coppel}
...
\citet[p.146]{blockcoppel} showed that $T_{\lambda^*}$ is chaotic in the sense of Li-Yorke.
\label{eg:counterexample}
\end{example}

\subsection*{Result}
It follows from Theorem~\ref{thm:devaney-blockcoppel} and Theorem~\ref{thm:entropy-blockcoppel} that \dev and \pte definitions are also equivalent.
Hence, \dev, \blcp, and \pte are equivalent to each other on a compact interval.
...
\begin{figure}[ht]
 ...
 %edges
    ...
    \draw[maybe] (entropy)[bend left=15] to node {?} (bc.east); 
    \draw[maybe] (dev)[bend left=15] to node {?} (bc.west); 
    \draw[maybe] (entropy)[bend right=15] to node {?} (dev); 
...
A system with positive topological entropy is chaotic in Li-Yorke's sense.
\begin{theorem}
  \citep{blanchard}
...
If $F$ is semi-conjugate to $\sigma: \Sigma \to \Sigma$, we have $h(\sigma) \leq h(F)$.
Since $h(\sigma)$ is positive, we conclude that $h(F)$ is positive.

\subsection*{Block-Coppel and Devaney/Wiggins}
...
Moreover, \citet{auslander} showed that $F$ restricted to the subset is also sensitive to initial conditions; hence, \blcp implies Wiggins.

On the other hand, \dev does not imply \blcp.
We can prove this by looking at the diagram.
Suppose \dev implies \blcp.
Then \dev implies \pte. However, this is not necessarily the case, as shown in \ref{eg:glasner}.

\subsection*{Block-Coppel and Li-Yorke}
\blcp implies \pte, and \pte implies \liy, so \blcp to \liy is induced.

\subsection*{Devaney and Topological Entropy}
\citet{glasner} provides an example of a system that is \dev but has zero entropy.
\begin{example}
  Some symbolic dynamics crap.
  \label{eg:glasner}
\end{example}
Hence, \dev does not imply \pte.
In the other direction, \citet{glasner} also proves that if a system is transitive, has no isolated point, and has positive topological entropy, then it is sensitive to initial conditions.
Thus, if we can prove that \pte implies the existence of a closed, invariant subset $Y \subseteq X$ on which $F$ is topologically transitive, then \pte implies \dev.

\subsection*{Devaney/Wiggins and Li-Yorke}
...
Does positive topological entropy imply the existence of a subset $Y$ such that $F_{|Y}$ is topological transitive?
Does topological transitivity imply positive topological entropy?
Is requiring dense periodic orbits too restrictive? Does periodicity have anything to do with chaos? We saw that a single periodic point + Wiggins is a sufficient condition for Li-Yorke. \citet{glasner} also propose that requiring a dense set of periodic points is too restrictive, and suggests alternative conditions, one of which is the existence of an invariant measure that is positive on each non-empty open set.
