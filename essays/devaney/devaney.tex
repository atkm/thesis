...
%However, these conditions are more or less the same, since the mapping is continuous.
Therefore, if there were an isolated point in the space, the dynamical system cannot be sensitive to initial conditions.
\end{remark}
We will use the following property of a topologically transitive map frequently.
\begin{proposition}
...
Since the properties of our interest are global properties of a space, we also require that the mapping be surjective. 
\begin{definition}
...
In this final section of the chapter, we introduce a definition that is equivalent to Wiggins's definition.
  ...
  \begin{definition}
    ...
    Suppose $F: X \to X$ is a continuous map, where $X$ is a compact subset of $\R^n$.
