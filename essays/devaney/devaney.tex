...
\begin{definition}
  ...
  Let $F: X \to X$.
    Suppose that $x_0$ is an unstable point such that $L(x_0) = X$,
...
In this final section of the chapter, we introduce a definition that is equivalent to Wiggins's definition.
  ...
  \begin{definition}
    ...
    The \textit{limit set} of $O(x_0)$, denoted $L(x_0)$, is the set of all limit points $O(x_0)$.
    \label{def:limset}
    \index{limit set}
  \end{definition}
  ...
  \begin{definition}
    (Unstable Point)
    Let $F: X\to X$ be an continuous mapping, and $x_0$ be a member of $X$.
  $x_0$ is called an \textit{unstable point}, if there exists $\epsilon > 0$, which we refer to as the \textit{separation constant} (as in the definition of sensitivity), such that, for any neighborhood $N$ of $x_0$, there exists $y_0 \in N$ and a positive integer $n$ such that 
  \begin{equation*}
    \metric{\itr{F}{n}(x_0), \itr{F}{n}(y_0)} > \epsilon.
  \end{equation*}
...
\begin{proposition}
  ...
  \begin{proof}
    ...
    \end{equation*}
    Since $F$ is topologically transitive, there exists a point $x_0 \in X$ such that some subsequence of its orbit visits each $A_{i,j}$ in this order.
    Now, choose any point $y \in X$.
    Since $\mathcal{C}_i$ is an open cover, for each $i$, there exists $j_i$ such that $y \in A_{i,j_i}$.
    We may introduce a subsequence $\set{\itr{F}{n(i)}(x_0)}$ of $O(x_0)$ such that, for each $i$, 
    \begin{equation*}
      \itr{F}{n(i)}(x_0) \in A_{i,j_i}.
    \end{equation*}
    By construction, $A_{i,j_i}$ is a sequence of sets whose diameters monotonically decrease. 
    It follows that $\set{\itr{F}{n(i)}(x_0)}$ converges to $y$.
    Since the choice of $y$ was arbitrary, we have $L(x_0) = X$.
  \end{proof}
\end{proposition}
%%%
Now we prove the second part of the theorem.
Trivially, if $F$ has a sensitive dependence on initial conditions on $X$, then for any $x_0 \in X$, $O(x_0)$ is unstable.
...
\begin{proposition}
  Suppose there exists $x_0 \in X$ such that $O(x_0)$ is unstable, and $L(x_0) = X$.
  Then, $F$ has a sensitive dependence on initial conditions.
  \label{prop:martelli-wiggins2}
  \begin{proof}
    Suppose that $x_0$ is an unstable point such that $L(x_0) = X$.
    Then, for each $y_0\in X$ and $\delta > 0$ there exists $n>0$ such that 
    \begin{equation*}
      \metric{\itr{F}{n}(x_0), y_0} \leq \frac{\delta}{2}.
    \end{equation*}
    Let $\epsilon(x_0)$ be its separation constant.
    We prove that $F$ has a sensitive dependence on initial conditions by considering two cases:
    \begin{enumerate}
      \item when there exists $p$ such that 
        \begin{equation*}
          \metric{\itr{F}{n+p}(x_0), \itr{F}{p}(y_0)} > \frac{\epsilon(x_0)}{2}
        \end{equation*}
      \item otherwise,
    \end{enumerate}
    and 
    In the other case, there exists a point $z_0 \in X (z_0 \neq y_0)$ in a neighborhood of $\itr{F}{n}(x_0)$ such that 
    \begin{equation*}
      \metric{\itr{F}{n}(x_0), z_0} \leq \frac{\delta}{2} 
      \quad \mbox{ and } \quad
      \metric{\itr{F}{n+p}(x_0), \itr{F}{p}(z_0)} > \epsilon(x_0).
    \end{equation*}
    Note that
    \begin{equation*}
      \metric{y_0, z_0} \leq \metric{y_0, \itr{F}{n}(x_0)} + \metric{\itr{F}{p}(x_0), z_0} \leq \delta,
    \end{equation*}
    by the triangle inequality.
    Moreover,
    \begin{align*}
      \epsilon(x_0) &< \metric{\itr{F}{p}(z_0), \itr{F}{n+p}(x_0)} \\
      &\leq \metric{\itr{F}{n+p}(x_0), \itr{F}{p}(y_0)} + \metric{\itr{F}{p}(y_0), \itr{F}{p}(z_0)}  \\
      &\leq \frac{\epsilon(x_0)}{2} + \metric{\itr{F}{n}(y_0), \itr{F}{n}(z_0)}
    \end{align*}
    Thus, in both cases we have
    \begin{equation*}
      \metric{\itr{F}{p}(y_0), \itr{F}{p}(z_0)} > \frac{\epsilon(x_0)}{2}.
    \end{equation*}
    Thus, $F$ has sensitive dependence on initial conditions with respect to $X$, and 
    $\epsilon(x_0)/2$ is the separation constant for any point in $X$.
  \end{proof}
\end{proposition}
