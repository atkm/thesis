...
\end{proposition}
%%%
Properies of $N(\alpha)$ propagate to $h(\alpha)$, which is defined as $h(\alpha) \equiv \log N(\alpha)$.
...
\begin{theorem}
  ...
  \begin{proof}[Proof of Theorem~\ref{thm:t-ent-conj}]
    ...
    \end{align*}
  %
    Since $\phi^{-1}(\alpha) < \alpha$, by Propositon~\ref{prop:hfa-props}(i), we obtain
    \begin{equation*}
      h(G,\alpha) 
      = h(F,\itr{\phi}{-1}(\alpha))
      \leq h(F, \alpha).
...
Since the logistic map is topologically conjugate (by a homeomorphism) to the full one-sided shift of two symbols $\set{0,1}$, our result from the previous chapter agrees with this property of shift systems.
